spp_params_fr<-function(trait_database_list,
  ...
  if(rebuild_species_list) {
    ...
    saveRDS(spp_fr_df, file = "data/fr/spp_fr_df.rds")
    rm(TAXREFv13)
    rm(metadonnees)
    gc()
    ...
    saveRDS(spp_fr_df_complete, file = "data/fr/spp_fr_df_complete.rds")
    write.table(spp_fr_df_complete, "data/fr/NFI_FR_mapping.csv", sep=";", na = "",
                row.names = FALSE)
  }  
  
  # SpParams initialization -------------------------------------------------
  cli::cli_h2("SpParamsFR initialisation")
  spp_fr_df_complete <- readRDS("data/fr/spp_fr_df_complete.rds")
  ...
  SpParams <- traits4models::init_medfate_params(spp_filt,
                                                 complete_rows = TRUE, 
                                                 verbose = FALSE)
  saveRDS(SpParams, file = "data/fr/SpParams_init_fr.rds")
  ...
  saveRDS(SpParams, file = "data/fr/SpParams_struct_fr.rds")
